...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        test = msg
        test = test.replace("makima", "Aco")
        test = test.replace("Makima", "Aco")
        response = await lunaQuery(
            test, message.from_user.id if message.from_user else 0
        )
        response = response.replace("Aco", "Makima")
        response = response.replace("aco", "Makima")

        pro = response
        try:
            await makima.send_chat_action(message.chat.id, "typing")
...
@makima.on_message(
    filters.regex("Makima|makima|makimabot|MakimaBot|Makimabot")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
